...
layout: home
---
In this blog, we have introduced the research done in syntax that addressed compositionality. Here we will dive deeper into details. 
...
```
        ...
        class Map:
            def __init__(self):
                self.units = [[Unit(f"{chr(65+i)}{j+1}") for j in range(3)] for i in range(3)]
...
```python
    ...
    class Map:
        def __init__(self, name):
            self.name = name
            self.units = [[Unit(f"{name}{chr(65+i)}{j+1}") for j in range(5)] for i in range(5)]
        ...
        def enforce_non_overlap(self, row, col):
            for i in range(5):
                if i != row:
                    self.units[i][col].active = False
                    print(f"Deactivating {self.units[i][col]} in {self.name} map due to 
